...
import { resolveAssetHref, isSensitiveResource } from '@/features/bdd-nsi/utils';
...
export default function BddNsiHubPage() {
  ...
  const [canSwitchToTeacher, setCanSwitchToTeacher] = React.useState<boolean>(false);
  ...
  const allDatasetResources = React.useMemo(
  ...
  const hideSolutions = state.role === 'eleve' || !state.showSolutions || !canSwitchToTeacher;
  const filteredDatasetResources = React.useMemo(
    () => hideSolutions
      ? allDatasetResources.filter((resource) => !isSensitiveResource(resource.href))
      : allDatasetResources,
    [allDatasetResources, hideSolutions]
  );
  const filteredTeacherDocs = React.useMemo(
    () => hideSolutions
      ? packBuilderTeacherDocs.filter((doc) => !isSensitiveResource(doc.href))
      : packBuilderTeacherDocs,
    [hideSolutions]
  );
  ...
  // Récupération du rôle réel depuis l'API pour empêcher l’accès enseignant aux élèves
  React.useEffect(() => {
    (async () => {
      try {
        ...
        if (res.ok && data?.ok) {
          if (data.role === 'TEACHER') {
            setCanSwitchToTeacher(true);
          } else {
            setCanSwitchToTeacher(false);
            // Force le rôle élève si l’utilisateur n’est pas enseignant
            setState((prev) => ({ ...prev, role: 'eleve', showSolutions: false }));
          }
        }
      } catch {
        setCanSwitchToTeacher(false);
  ...
  return (
    <Layout
      ...
      right={<div className="flex items-center gap-2">
        ...
        <Button
          variant={state.role === 'enseignant' ? 'primary' : 'ghost'}
          onClick={() => updateRole('enseignant')}
          disabled={!canSwitchToTeacher}
          title={canSwitchToTeacher ? 'Basculer vers l’espace enseignant' : 'Réservé aux enseignants'}
        >
          Espace enseignant
        </Button>
      </div>}
    >
      <div className="space-y-12">
        ...
        {state.role === 'enseignant' && canSwitchToTeacher && (
          <>
            ...
            <Section id="enseignant-ressources" title="Bibliothèque enseignant" description="Imports SQL, corrections et scripts de vérification.">
              <div className="grid gap-4 md:grid-cols-3">
                {teacherResourceGroups.map((group) => {
                  const items = hideSolutions
                    ? group.items.filter((item) => !isSensitiveResource(item.href))
                    : group.items;
                  return (
                    <Card key={group.title} title={group.title} subtitle={group.description}>
                      {items.length > 0 ? (
                        ...
                        <p className="text-sm text-[var(--fg)]/60">Activez l’affichage des solutions pour accéder aux corrections.</p>
                      )}
                    </Card>
                  );
                })}
            ...
            <Section id="pack-builder" title="Pack builder" description="Préparez un pack autonome (HTML + assets) à partager aux élèves.">
              ...
              <Card title="Assets inclus" subtitle="Ressources empaquetées pour le dataset choisi.">
                <ul className="space-y-1">
                  {filteredDatasetResources.map((resource) => (
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <Card title="Docs enseignants">
                  {filteredTeacherDocs.length > 0 ? (
                    ...
                    <p className="text-sm text-[var(--fg)]/60">Activez les solutions pour inclure corrections et barèmes dans le pack.</p>
                  )}
                </Card>
              </div>

              {state.role === 'enseignant' && hideSolutions && (
                <p className="text-xs text-[var(--fg)]/60">Les ressources sensibles (solutions, corrections) sont masquées tant que les solutions ne sont pas affichées.</p>
              )}
              ...
              <Card key={group.category} title={group.category}>
                {(() => {
                  const items = hideSolutions
                    ...
                    : group.items;
                  if (items.length === 0) {
                    return <p className="text-sm text-[var(--fg)]/60">Solutions et corrections masquées. Activez-les pour les afficher.</p>;
                  }
                  return (
                    <ul className="space-y-1">
                      {items.map((item) => (
