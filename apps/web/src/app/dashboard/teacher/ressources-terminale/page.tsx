...
export default function TeacherResourcesTermPage() {
  ...
  React.useEffect(() => {
    (async () => {
      try {
        const res = await fetch(`/api/ressources/terminale?v=${Date.now()}`, { cache: 'no-store' });
  ...
  return (
    ...
    >
      <div className="space-y-4">
        <h1 className="text-2xl">Ressources Terminale</h1>
        <div className="rounded-xl overflow-hidden border border-white/10" style={{ height: '80vh' }}>
          <iframe title="Ressources NSI Terminale" srcDoc={srcDoc} className="w-full h-full bg-white" />
        </div>
      </div>
    </Layout>
  );
